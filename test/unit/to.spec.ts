...
describe("output tests", () => {
    it("should create an array", () => {
        const array = fromIterable([1, 2, 3, 4, 5])
            .where((_) => _ < 3)
            .toArray();
    ...
    [range(0, 10), range(0, 10).toArray()].forEach((source, indx) => {
        it(
            "to array should map elements if map function is provided: " + indx,
            () => {
                const iterable = fromIterable(source);
                const arr = iterable.toArray((_) => _ * 2);
                let i = 0;
                for (const item of range(0, 10)) {
                    expect(arr[i]).toBe(item * 2);
                    i++;
                }
            },
        );
