...
import { insertOrdered, search } from "../../src/utils.ts";
...
describe('utils functions tests', () => {
    it('should search in ordered array', () => {
        ...
        expect(search(array, 'z', comparer)).toBe(-1);
        expect(search(array, '!', comparer)).toBe(-1);
    });
    ...
    it('should be able to insert values in ordered array and to keep order', () => {
        ...
        expect(array).toEqual([3, 6]);
        insertOrdered(array, 7, comparer);
        expect(array).toEqual([3, 6, 7]);
        insertOrdered(array, 4, comparer);
        expect(array).toEqual([3, 4, 6, 7]);
        insertOrdered(array, 4, comparer);
        expect(array).toEqual([3, 4, 4, 6, 7]);
        insertOrdered(array, 0, comparer);
        expect(array).toEqual([0, 3, 4, 4, 6, 7]);
        insertOrdered(array, 5, comparer);
        expect(array).toEqual([0, 3, 4, 4, 5, 6, 7]);
        insertOrdered(array, 7, comparer);
        expect(array).toEqual([0, 3, 4, 4, 5, 6, 7, 7]);
        insertOrdered(array, 0, comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7]);
        insertOrdered(array, 'a', comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a']);
        insertOrdered(array, 'c', comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a', 'c']);
        insertOrdered(array, 'b', comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a', 'b', 'c']);
        insertOrdered(array, 'b', comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a', 'b', 'b', 'c']);
        insertOrdered(array, undefined, comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a', 'b', 'b', 'c', undefined]);
        insertOrdered(array, null, comparer);
        expect(array).toEqual([0, 0, 3, 4, 4, 5, 6, 7, 7, 'a', 'b', 'b', 'c', null, undefined]);
    });
});
